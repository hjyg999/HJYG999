...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends BarLineScatterCandleBubbleDataSet<? extends Entry>>>
     ...
     */
    private boolean mAutoScaleMinMaxEnabled = true;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mXAxisRenderer.renderAxisLine(canvas);//绘制沿轴的线。
        ...
        if(isDrawlable){
        	mLegendRenderer.renderLegend(canvas);//画下面的标签，如 一周收益
        }
        drawMarkers(canvas);
...
//            mAxisRight.setValueFormatter(mDefaultFormatter);
     ...
     * 设置是否画标签
     * */
    public void setisDrawlable(boolean enabled) {
    	this.isDrawlable = enabled;
    }
