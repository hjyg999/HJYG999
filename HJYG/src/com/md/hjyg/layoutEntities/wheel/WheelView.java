...
//	private static final int[] SHADOWS_COLORS = new int[] { 0xeeffffff, 0xeaffffff, 0x33ffffff };
	private  int[] SHADOWS_COLORS = new int[] {  0xaaffffff,0x77ffffff,0x33ffffff };
...
//		initData(context);
	...
	public void setAddress(boolean isAddress){
		...
		if (isAddress) {
			SHADOWS_COLORS = new int[] {  0x00ffffff,0x00ffffff,0x00ffffff };
		}
	}
...
//		centerDrawable.draw(canvas);
		...
		if (lable != null && lable.length()>0) {
			...
			int px = ScreenUtils.sp2px(context, 14) - 4;
			paint.setTextSize(px );
			if (lable.equals("年")) {
				canvas.drawText(lable, getWidth()/2+px*2f, center+px/2 - 1, paint);
			}else {
				canvas.drawText(lable, getWidth()/2+px*1, center+px/2 - 1, paint);
			}
		}
	}
