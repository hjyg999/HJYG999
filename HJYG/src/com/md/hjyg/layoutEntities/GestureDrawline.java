...
import com.md.hjyg.entity.GesturePoint;
...
//	private int[] screenDispaly;
	 ...
	 * 是否画中间点
	 */
	private boolean isDrawbetweenPoint ;
...
//		screenDispaly = AppUtil.getScreenDispaly(context);
		screenDispaly = (int) (ScreenUtils.getScreenHeight(context)*0.478125);
		paint = new Paint(Paint.DITHER_FLAG);// 创建一个画笔
//		bitmap = Bitmap.createBitmap(screenDispaly[0], screenDispaly[0],
//				Bitmap.Config.ARGB_8888); // 设置位图的宽高
		bitmap = Bitmap.createBitmap(screenDispaly, screenDispaly,
				Bitmap.Config.ARGB_8888); // 设置位图的宽高
		canvas = new Canvas();
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (pointAt == null
				...
				//|| Constants.POINT_STATE_SELECTED == pointAt.getPointState()或者当前点击的点处于选中状态
				...
				GesturePoint betweenPoint = getBetweenCheckPoint(currentPoint,
						pointAt);
				if (betweenPoint != null && isDrawbetweenPoint
		...
		case MotionEvent.ACTION_UP:// 当手指抬起的时候
			if (isVerify) {
				...
				if (passWord.equals(Constants.md5(passWordSb.toString()))) {
					// 代表用户绘制的密码手势与传入的密码相同
					callBack.checkedSuccess();
				} else {
					// 用户绘制的密码与传入的密码不同。
					count = callBack.checkedFail();
					if(count == 5)
					{
						clearDrawlineState(200L);
					}
				}
			} else {
	...
	/**
	 * 指定时间去清除绘制的状态 ,连续绘制5次错误，锁屏，不能再绘制
	 * 
	 * @param delayTime
	 *            延迟执行时间
	 */
	...
	final class clearStateRunnable implements Runnable {
		public void run() {
			...
			for (GesturePoint p : list) {
				if (count == 5) {
					p.setPointState(Constants.POINT_STATE_WRONG);
				} else {
	...
	private void drawErrorPathTip() {
		...
		paint.setColor(context.getResources().getColor(R.color.yellow_ff9933));
		for (Pair<GesturePoint, GesturePoint> pair : lineList) {
	...
	public interface GestureCallBack {
		 ...
		 * 代表用户绘制的密码与传入的密码不相同
		 */
		public abstract int checkedFail();
	}
